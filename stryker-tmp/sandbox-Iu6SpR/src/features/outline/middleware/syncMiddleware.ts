...
const handleCharacterDeletionCleanup = (store: any, characterId: string, state: any) => {
  ...
  for (let i = 0; i < updatedChapters.length; i++) {
    if (
      JSON.stringify(updatedChapters[i].keyScenes) !==
      JSON.stringify(state.chapters.chapters[i].keyScenes)
    ) {
  ...
  for (let i = 0; i < updatedSubplots.length; i++) {
    if (
      JSON.stringify(updatedSubplots[i].relatedCharacters) !==
      JSON.stringify(state.subplots.subplots[i].relatedCharacters)
    ) {
  ...
  if (updatedSecondaryStories.length !== state.subplots.secondaryStories.length) {
    ...
    state.subplots.secondaryStories
      ...
      .forEach((story: any) => {
        ...
        console.warn(
          `Secondary story ${story.id} needs cleanup for deleted character ${characterId}`
        );
      });
  }
  ...
  const updatedIdeas = state.ideas.ideas.map((idea: any) => ({
    ...idea,
    relatedElements: idea.relatedElements.filter(
  ...
  for (let i = 0; i < updatedIdeas.length; i++) {
    if (
      JSON.stringify(updatedIdeas[i].relatedElements) !==
      JSON.stringify(state.ideas.ideas[i].relatedElements)
    ) {
      ideasChanged = true;
      break;
    }
...
const handleChapterReorderingSync = (store: any, chapterMapping: any, state: any) => {
  ...
  for (let i = 0; i < updatedSubplots.length; i++) {
    if (
      updatedSubplots[i].startChapter !== state.subplots.subplots[i].startChapter ||
