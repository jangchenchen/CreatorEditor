...
const EventCard: React.FC<EventCardProps> = ({ event, characters, onEdit, onDelete }) => {
  const handleDelete = () => {
      ...
      <CardContent>
        <Box sx={{ display: 'flex', alignItems: 'flex-start', mb: 1 }}>
          <Typography variant='h6' sx={{ flexGrow: 1 }}>
            {event.title}
            {event.isKeyEvent && <StarIcon color='secondary' sx={{ ml: 1, fontSize: 20 }} />}
          </Typography>
          ...
          <Box sx={{ mb: 1 }}>
            ...
            <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5, mt: 0.5 }}>
              ...
              {event.characters.length > 3 && (
                <Chip label={`+${event.characters.length - 3}`} size='small' variant='outlined' />
              )}
            </Box>
          </Box>
        )}
