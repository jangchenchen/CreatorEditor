...
import { ValidationResult, ValidationError, ValidationWarning, CleanupAction } from './integrityTypes';
...
export class ReferentialIntegrityServiceNew {
  ...
  static validateState(state: OutlineState): ValidationResult {
    ...
    // Run all validators
    TimelineValidator.validateTimelineEvents(state.timeline.events, validCharacterIds, errors, warnings);
    ChapterValidator.validateChapterScenes(state.chapters.chapters, validCharacterIds, errors, warnings);
    SubplotValidator.validateSubplots(state.subplots.subplots, validCharacterIds, validChapterNumbers, errors, warnings);
    SecondaryStoryValidator.validateSecondaryStories(state.subplots.secondaryStories, validCharacterIds, errors, warnings);
    IdeaValidator.validateIdeas(state.ideas.ideas, validCharacterIds, validChapterIds, errors, warnings);
