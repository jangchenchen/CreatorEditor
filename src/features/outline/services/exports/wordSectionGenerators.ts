...
export class WordSectionGenerators {
  ...
  static generateStorySection(data: OutlineData): Paragraph[] {
    ...
    return [
      ...
      this.createBoldParagraph("背景设定", `${story.background.era} - ${story.background.location}`),
  ...
  static generateTimelineSection(events: PlotEvent[]): Paragraph[] {
    ...
    events.forEach(event => {
      paragraphs.push(
        new Paragraph({ text: `${event.timestamp} - ${event.title}`, heading: HeadingLevel.HEADING_2 }),
  ...
  static generateChaptersSection(chapters: Chapter[]): Paragraph[] {
    ...
    chapters.forEach(chapter => {
      paragraphs.push(
        new Paragraph({ text: `第${chapter.number}章: ${chapter.title}`, heading: HeadingLevel.HEADING_2 }),
      ...
      if (chapter.keyScenes.length > 0) {
        ...
        chapter.keyScenes.forEach((scene, index) => {
          paragraphs.push(new Paragraph({ text: `${index + 1}. ${scene.title}: ${scene.description}`, style: "Normal" }));
        });
      }
  ...
  static generateSubplotsSection(subplots: Subplot[]): Paragraph[] {
    ...
    subplots.forEach(subplot => {
      paragraphs.push(
        ...
        this.createBoldParagraph("章节范围", `第${subplot.startChapter}章 - 第${subplot.endChapter}章`),
        new Paragraph({ text: "" })
      );
    });
  ...
  private static translate(type: string, key: string): string {
    const translations = {
      ...
      chapterStatus: { 'planned': '计划中', 'writing': '写作中', 'completed': '已完成', 'revision': '修订中' },
      subplotPurpose: { 'background': '背景补充', 'contrast': '对比衬托', 'suspense': '悬念制造', 'character-development': '角色发展', 'comic-relief': '喜剧调节' },
      subplotStatus: { 'planned': '计划中', 'active': '进行中', 'resolved': '已解决', 'abandoned': '已放弃' },
      ideaType: { 'inspiration': '灵感', 'plot-extension': '情节延伸', 'alternative-ending': '替代结局', 'scene-idea': '场景想法', 'character-twist': '角色转折', 'dialogue': '对话' },
      ideaStatus: { 'draft': '草稿', 'considering': '考虑中', 'adopted': '已采用', 'rejected': '已拒绝', 'archived': '已归档' }
    };
