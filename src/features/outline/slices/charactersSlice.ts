...
const charactersSlice = createSlice({
  ...
  reducers: {
    ...
    updateCharacter: (state, action: PayloadAction<{ id: string; updates: Partial<Character> }>) => {
      ...
      if (index !== -1) {
        state.characters[index] = { ...state.characters[index], ...action.payload.updates, lastUpdated: new Date() };
      }
    },
    ...
    updateRelationship: (state, action: PayloadAction<{ id: string; updates: Partial<Relationship> }>) => {
      const index = state.relationships.findIndex(r => r.id === action.payload.id);
      if (index !== -1) {
        state.relationships[index] = { ...state.relationships[index], ...action.payload.updates, lastUpdated: new Date() };
      }
    },
...
// Selectors
export const selectCharacters = (state: { outline: { characters: CharactersState } }) => state.outline.characters.characters;
export const selectRelationships = (state: { outline: { characters: CharactersState } }) => state.outline.characters.relationships;

export const selectCharacterById = (characterId: string) => 
  (state: { outline: { characters: CharactersState } }) => 
...
export const selectCharactersStats = (state: { outline: { characters: CharactersState } }) => ({
  ...
  protagonistCount: state.outline.characters.characters.filter(c => c.role === 'protagonist').length,
